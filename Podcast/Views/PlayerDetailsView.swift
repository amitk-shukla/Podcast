...
class PlayerDetailsView: UIView {
  ...
  fileprivate func setupRemoteControl() {
    ...
    commandCenter.nextTrackCommand.addTarget { (event) -> MPRemoteCommandHandlerStatus in
      ...
      let currentEpisodeIndex = self.playlistEpisodes.index { (ep) -> Bool in
    ...
    commandCenter.previousTrackCommand.addTarget { (event) -> MPRemoteCommandHandlerStatus in
      if self.playlistEpisodes.isEmpty {
        return .commandFailed
      }
      ...
      if index == 0 {
        let count = self.playlistEpisodes.count
        prevEpisode = self.playlistEpisodes[count - 1]
      } else {
    ...
    // This code was crashing in iOS 13
